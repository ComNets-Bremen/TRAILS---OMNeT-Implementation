...
def classifyPaths(POIs,recursion):
    lenPOIs=len(POIs);
    if recursion != None:
        if recursion>lenPOIs:
            realRecursion=recursion+SAFE_TRHESHOLD;
            sys.setrecursionlimit(realRecursion);       
            searchReturnLinksWithoutU(POIs,recursion);
            searchReturnLinksWithU(POIs,recursion);
            sys.setrecursionlimit(DEFAULT_RECURSION);
    else:
        realRecursion=lenPOIs+SAFE_TRHESHOLD;
        sys.setrecursionlimit(realRecursion);       
        searchReturnLinksWithoutU(POIs,lenPOIs);
        searchReturnLinksWithU(POIs,lenPOIs);
        sys.setrecursionlimit(DEFAULT_RECURSION);        
    countPaths(POIs);
